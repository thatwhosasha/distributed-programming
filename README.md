...
## Работа с Redis(Docker)
|Действие|Команда|
|---------------------------------|------------------------------------------------|
| Запустить новый контейнер Redis|	`docker run --name redis -p 6379:6379 -d redis`|
| Запустить существующий контейнер (если был остановлен)|	`docker start redis`|
| Остановить контейнер|	`docker stop redis`|
| Удалить контейнер (предварительно остановить)|	`docker rm redis`|
| Проверить, запущен ли Redis|	`docker ps`|
...
|Проверить работоспособность (PING)|	`docker exec -it redis redis-cli PING` Ожидаемый ответ: `PONG`|
